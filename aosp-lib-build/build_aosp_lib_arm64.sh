...
OH=$WLROOT/openharmony; A=$WLROOT/aosp-15; NDK=$WLROOT/ohos-sdk-6.1/linux/native; SR=$NDK/sysroot
CXX=$OH/prebuilts/clang/ohos/linux-x86_64/llvm/bin/clang++; CC=$OH/prebuilts/clang/ohos/linux-x86_64/llvm/bin/clang
O=$WLROOT/bridge-build-arm64/aosp_lib; mkdir -p $O
BUILTINS=$(find $OH/prebuilts/clang/ohos -path '*aarch64-linux-ohos*builtins.a' 2>/dev/null | head -1)
PSDK=$WLROOT/bridge-build-arm64/out/board_libs   # board libs (hilog, z) to link against
...
COMMON="--target=aarch64-linux-ohos --sysroot=$SR -fPIC -O2 -DNDEBUG -D__OHOS__ -D_GNU_SOURCE -include $WLROOT/bridge-build-arm64/musl_compat.h $WARN"
CXXF="$CXX $COMMON -std=gnu++20"; CF="$CC $COMMON -std=gnu11 -D__ANDROID_API__=34"
LNK="$CXX --target=aarch64-linux-ohos -B$WLROOT/ohos-sdk-6.1/linux/native/sysroot/usr/lib/aarch64-linux-ohos -L$WLROOT/ohos-sdk-6.1/linux/native/sysroot/usr/lib/aarch64-linux-ohos -L$O -L$PSDK -shared -fPIC"
bld() {
...
# 5. ziparchive
EXTRA_LIBS="-lbase -llog -lz" bld ziparchive "-DZLIB_CONST -I$A/system/libziparchive/include -I$A/system/libziparchive/incfs_support/include -I$WLROOT/bridge-build-arm64/stubinc -I$A/system/libbase/include -I$A/system/logging/liblog/include -I$A/system/core/include -I$A/external/zlib" \
