...
OVL=$WLROOT/bridge-build-arm64/overlay; STUB=$WLROOT/bridge-build-arm64/stubinc
O=$WLROOT/bridge-build-arm64/bridge_build; OUT=$WLROOT/bridge-build-arm64/out; LIBDIR=$OUT/board_libs; mkdir -p $O $OUT
INCS="-I$OVL/inc -I$STUB $(cat $WLROOT/bridge-build-arm64/bridge_incs_all.txt)"
...
ok=0; fl=0; OBJS=""
for src in $(find $WLROOT/bridge-build/src -name '*.cpp' | grep -v appspawn-x); do
