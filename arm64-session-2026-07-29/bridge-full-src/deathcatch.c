...
/* GCC-style frame registration symbols are referenced weakly by crt startup on some
 * toolchains; if unresolved they call through null -> SIGSEGV on dlopen. Provide no-op
 * definitions (LLVM libunwind registers FDEs itself, so these are safe stubs). */
void __register_frame_info(const void* b, void* o) { (void)b; (void)o; }
void __deregister_frame_info(const void* b) { (void)b; }
void __register_frame_info_bases(const void* b, void* o, void* t, void* d) { (void)b;(void)o;(void)t;(void)d; }
...
/* ---- raw syscall interposer (catch exit_group=94/exit=93 via libc syscall()) ----
 * Forward via an inline svc (NOT dlsym'd real — dlsym re-enters syscall -> infinite
 * recursion -> SIGSEGV on load). A re-entry guard keeps our own logging (write) from
 * recursing through the exit-number check. */
static long raw_syscall(long n, long a0, long a1, long a2, long a3, long a4, long a5) {
...
long syscall(long num, ...) {
    ...
    va_end(ap);
    if (!g_in_hook && (num == 94 || num == 93 || num == 248 || num == 1)) {
        g_in_hook = 1;
        logline("syscall_exit_group", CALLER(), a0, "code");
        log_backtrace("sc_exit");
        g_in_hook = 0;
    }
    return raw_syscall(num, a0, a1, a2, a3, a4, a5);
}
...
static void dc_init(void) {
    ...
    /* keep g_maps_dumped=0 across fork so each child dumps its OWN maps on its
     * first hook (needed to resolve the caller address). One tiny load marker: */
    char b[48]; int n = 0;
    const char* t = "[DEATHCATCH] loaded pid=";
    for (const char* p = t; *p; p++) b[n++] = *p;
    numcat(b, &n, (long)getpid());
    b[n++] = '\n';
    write(DC_FD, b, n);
