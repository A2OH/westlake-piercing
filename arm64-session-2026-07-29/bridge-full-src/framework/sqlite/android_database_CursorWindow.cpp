...
static jlong nativeCreate(JNIEnv* env, jclass clazz, jstring nameObj, jint cursorWindowSize) {
    ...
    // WESTLAKE §452: the 2MB the framework asks for is fine — the real bug was that the window
    // could never inflate past its 16KB inline buffer (see §453 in wl_CursorWindow.cpp).
...
fail:
    jniThrowExceptionFmt(env, "android/database/CursorWindowAllocationException",
                         ...
                         "<cursorwindow>", cursorWindowSize, status);
    return 0;
}

static jlong nativeCreateFromParcel(JNIEnv* env, jclass clazz, jobject parcelObj) {
    Parcel* parcel = nullptr;  // WESTLAKE: parcelForJavaObject unavailable on this board
    (void)parcelObj;
...
static jstring nativeGetName(JNIEnv* env, jclass clazz, jlong windowPtr) {
    CursorWindow* window = reinterpret_cast<CursorWindow*>(windowPtr);
    return env->NewStringUTF("<cursorwindow>");
}
