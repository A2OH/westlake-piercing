...
status_t CursorWindow::maybeInflate() {
    ...
    if (false) {
        // WESTLAKE §453: OHOS has no ashmem, so the window could never grow past its 16KB inline
        // buffer and every row bigger than that failed with
        //   SQLiteBlobTooBigException: Row too big to fit into CursorWindow
        // (noice's sound_metadata rows reach 114KB). The window is only consumed in-process here —
        // nothing hands it across Binder — so a heap buffer is a correct substitute.
        {
            void* heapData = malloc(mInflatedSize);
            if (heapData == nullptr) {
                PLOG(ERROR) << "Failed ashmem_create_region and malloc fallback";
                goto fail_silent;
            }
            const uint32_t slotsSize = sizeOfSlots();
            const uint32_t newSlotsOffset = mInflatedSize - slotsSize;
            memcpy(static_cast<uint8_t*>(heapData),
                    static_cast<uint8_t*>(mData), mAllocOffset);
            memcpy(static_cast<uint8_t*>(heapData) + newSlotsOffset,
                    static_cast<uint8_t*>(mData) + mSlotsOffset, slotsSize);
            free(mData);
            mAshmemFd = -1;              // destructor then free()s instead of munmap()ing
            mData = heapData;
            mSize = mInflatedSize;
            mSlotsOffset = newSlotsOffset;
            updateSlotsData();
            fprintf(stderr, "[WESTLAKE-453] CursorWindow inflated to %zu bytes via malloc "
                    "(no ashmem on this platform)\n", mInflatedSize);
            fflush(stderr);
            return OK;
        }
    }
...
heap_fallback:
    // WESTLAKE §453: OHOS has no working ashmem, so the window could never grow past its 16KB
    // inline buffer and every larger row failed with
    //   SQLiteBlobTooBigException: Row too big to fit into CursorWindow
    // (noice's sound_metadata rows reach 114KB, which is why the sound library never rendered).
    // Nothing hands this window across Binder in our single-process setup, so a heap buffer is a
    // correct substitute. Covers EVERY ashmem failure mode, not just a failed create_region.
    {
        if (newData != nullptr && newData != MAP_FAILED) { ::munmap(newData, mInflatedSize); }
        if (ashmemFd > 0) { ::close(ashmemFd); }
        newData = nullptr;
        ashmemFd = -1;
        void* heapData = malloc(mInflatedSize);
        if (heapData == nullptr) {
            LOG(ERROR) << "Failed maybeInflate: malloc fallback";
            return NO_MEMORY;
        }
        const uint32_t slotsSize = sizeOfSlots();
        const uint32_t newSlotsOffset = mInflatedSize - slotsSize;
        memcpy(static_cast<uint8_t*>(heapData), static_cast<uint8_t*>(mData), mAllocOffset);
        memcpy(static_cast<uint8_t*>(heapData) + newSlotsOffset,
                static_cast<uint8_t*>(mData) + mSlotsOffset, slotsSize);
        free(mData);
        mAshmemFd = -1;                 // destructor free()s instead of munmap()ing
        mData = heapData;
        mSize = mInflatedSize;
        mSlotsOffset = newSlotsOffset;
        updateSlotsData();
