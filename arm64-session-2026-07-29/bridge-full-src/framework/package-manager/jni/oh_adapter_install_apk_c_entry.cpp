    ...
    char* outJsonBuf, int outJsonBufSize)
{
    ...
    for (const auto& ability : manifest.activities) {
        ...
        a["launchMode"]    = (ability.launchMode == 2 || ability.launchMode == 3) ? 0 : 1; /* [LM-FIX] Android launchMode -> never OHOS SPECIFIED(2): singleTask/singleInstance->SINGLETON(0) else STANDARD(1) */
