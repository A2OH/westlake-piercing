...
// /data/local/tmp/asx/fw/framework.jar persists from prior deploys, the
...
static const char kBootClasspath[] =
    "/data/local/tmp/asx/fw/core-oj.jar:"
    "/data/local/tmp/asx/fw/core-libart.jar:"
    "/data/local/tmp/asx/fw/core-icu4j.jar:"
    "/data/local/tmp/asx/fw/okhttp.jar:"
    "/data/local/tmp/asx/fw/bouncycastle.jar:"
    "/data/local/tmp/asx/fw/apache-xml.jar:"
    ...
    "/data/local/tmp/asx/fw/framework.jar:"
    "/data/local/tmp/asx/fw/adapter-runtime-bcp.jar:"
...
static void seedRequiredEnvs() {
    ...
    setenv("ANDROID_BOOT_IMAGE",  "/data/local/tmp/asx/fw/boot.art",    1);
    // WESTLAKE (arm64 board, 2026-07-21): appspawn-x creates its OWN mount-ns sandbox,
    // so forked children CANNOT see the real /system — only /data/local/tmp is bind-mounted
    // in.  ICU4J (core-icu4j.jar, icudt75b) resolves its data via ANDROID_I18N_ROOT/etc/icu,
    // so with the old /system/android value the child got
    //   MissingResourceException -> android.icu.impl.locale.LikelySubtags clinit fails
    //   -> android.graphics.Typeface clinit NPEs -> sDefaultTypeface null
    //   -> ensureBindApplication fails -> mInitialApplication null -> getResources() NPE.
    // Point it at the sandbox-visible staging dir instead (same reason framework-res.apk
    // had to be redirected to /data/local/tmp/asx).
    setenv("ANDROID_I18N_ROOT",   "/data/local/tmp/asx/i18n",              1);
    setenv("ANDROID_TZDATA_ROOT", "/system/android",                       1);

    // ICU data path - read by ICU u_init() during Runtime::Start
    setenv("ICU_DATA",            "/data/local/tmp/asx/i18n/etc/icu",      1);
...
int main(int argc, char* argv[]) {
    ...
    server.setSpawnHandler([&](const SpawnMsg& msg) -> int {
        ...
        if (pid == 0) {
            ...
            // WESTLAKE 2026-07-22 (§146): the child reaps as exited(1) with NO fatal signal and
            // WITHOUT child_main.cpp:305's [CM-EXIT] marker, so identify whether THIS is the exit.
            fprintf(stderr, "[WESTLAKE-MAINEXIT] ChildMain::run returned -> main.cpp _exit(1)\n");
            fflush(nullptr);
            _exit(1);
        }
