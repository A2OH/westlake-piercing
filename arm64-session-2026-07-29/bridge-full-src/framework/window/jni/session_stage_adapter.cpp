    ...
    : jvm_(jvm)
{
    ...
    jclass localBridgeClass = env->FindClass(
        // WESTLAKE §284o — WRONG CLASS NAME (one-line blocker for the whole V7 path).
        // The class injected on the BCP is `adapter/window/SessionStageBridge` (present in BOTH
        // apache-xml.jar:classes4.dex, the §233 injection, and oh-adapter-framework.jar:classes.dex)
        // -- there is no `adapter/bridge/callback/` package anywhere.  So FindClass failed,
        // SessionStageAdapter's ctor threw NoClassDefFoundError, createSession caught it and
        // "fell back" to the LEGACY IWindowManager::CreateWindow, which OH 6.1 does not serve
        // (ret=1005 WM_ERROR_IPC_FAILED, §284n).  No session => the surface node gets spliced onto
        // LOGICAL_DISPLAY_NODE by AttachToDisplay instead of hanging under a CANVAS_NODE (§284l)
        // => RS keeps it on the tree, takes its buffers, but never composites => BLACK SCREEN.
...
{
    fprintf(stderr, "[WESTLAKE-STAGE] SetActive\n"); fflush(stderr);
...
{
    fprintf(stderr, "[WESTLAKE-STAGE] NotifySessionForeground\n"); fflush(stderr);
...
{
    fprintf(stderr, "[WESTLAKE-STAGE] NotifyDestroy\n"); fflush(stderr);
...
{
    fprintf(stderr, "[WESTLAKE-STAGE] NotifyWindowVisibility\n"); fflush(stderr);
...
{
    fprintf(stderr, "[WESTLAKE-STAGE] UpdateRect\n"); fflush(stderr);
...
{
    fprintf(stderr, "[WESTLAKE-STAGE] UpdateFocus\n"); fflush(stderr);
