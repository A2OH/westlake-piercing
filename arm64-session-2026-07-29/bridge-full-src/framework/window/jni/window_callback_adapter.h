...
// 2026-07-22 (arm64): these two MUST come BEFORE window_stub.h.  window_stub.h pulls in OHOS's
// zidl/window_interface.h, which references OccupiedAreaChangeInfo and MMI::KeyEvent without
// including their headers itself (upstream relies on BUILD.gn deps to supply them).  Without
// these, window_interface.h fails to parse and this whole TU is silently dropped from the link
// (the "OBJS only collects TUs that compile" trap) — which is why no window is ever added.
