                                     ...
                                     int32_t width, int32_t height, int32_t format)
{
    ...
    // 2026-06-04 NEW-SURFACE EGL FIX: make EVERY producer surface GPU-renderable
    // at creation. The G3.4b fix only set HW_RENDER usage + format at hwui-bind
    // time (getOhNativeWindow) for the FIRST window; a SECOND window/surface (a
    // dialog/time-picker opened from e.g. the sleep-timer or add-alarm flow)
    // could reach eglCreateWindowSurface with defFormat=0 (UNSET) + defUsage
    // lacking HW_RENDER(0x100) → EGL returns EGL_NO_SURFACE → hwui CanvasContext
    // aborts ("no surface") → the render thread dies → RSClientToRenderConnection
    // OnRemoteDied → process crash. Applying the format/usage to the producer
    // surface here covers every surface from birth, so 2nd-window EGL creation
    // succeeds. (Same SetDefaultUsage|0x100|0x200 + SetDefaultFormat(12) the
    // G3.4b getOhNativeWindow path uses; harmless to set early/again.)
    {
        uint64_t du = ohSurface->GetDefaultUsage();
        auto suErr = ohSurface->SetDefaultUsage(du | 0x100ULL | 0x200ULL);  // +HW_RENDER +HW_TEXTURE
        auto sfErr = ohSurface->SetDefaultFormat(12);  // GRAPHIC_PIXEL_FMT_RGBA_8888
        LOGI("createSurface[NEW-SURFACE-EGL-FIX]: session=%d prevUsage=0x%llx SetDefaultUsage rc=%d "
             "SetDefaultFormat(12) rc=%d newUsage=0x%llx newFormat=%d",
             sessionId, static_cast<unsigned long long>(du), (int)suErr, (int)sfErr,
             static_cast<unsigned long long>(ohSurface->GetDefaultUsage()),
             ohSurface->GetDefaultFormat());
    }
