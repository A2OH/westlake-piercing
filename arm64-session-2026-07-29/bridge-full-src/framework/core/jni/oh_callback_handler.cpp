...
void OHCallbackHandler::ensureJavaVM(JavaVM* vm)
{
    if (jvm_ == nullptr && vm != nullptr) {
        jvm_ = vm;
        LOGI("ensureJavaVM: captured process JavaVM for AbilityConnection use");
    }
}
    ...
    int connectionId)
{
    JavaVM* vm = jvm_;
    if (vm == nullptr) {
        // [FAV-FIX 2026-06-30] registerCallbacks() is sometimes invoked with no
        // JavaVM (oh_environment.cpp path), leaving jvm_ null; a service bind that
        // arrives before a VM-carrying registerCallbacks (noice Presets/Favorites ->
        // SoundPlaybackService) then crashed with "JavaVM is null". Fall back to the
        // process's live VM via the standard JNI_GetCreatedJavaVMs (libart export) —
        // no cached pointer or cross-.so symbol needed.
    ...
    if (vm == nullptr) {
        LOGE("Cannot create AbilityConnection: JavaVM is null (no jvm_, no created VM)");
        return nullptr;
    }
    return new AbilityConnectionAdapter(vm, connectionId);
}
