...
//
// 2026-05-18 (minimal isolation experiment): re-introduce the complete
// Parcelable class definition, while keeping the 3 AOSP sources
// (MotionEvent.cpp / Input.cpp / InputEventLabels.cpp) OUT of SRCS_AOSP.
// Goal: determine whether #1 alone (Parcelable complete-class) introduces
// any runtime regression vs the bkup-0513-UI baseline.  If UI still
// displays, #1 is harmless on its own; if UI breaks, #1 is the culprit
// inside the (#1 + 3 AOSP source) group from earlier experiments.
namespace android {
class IInterface;
