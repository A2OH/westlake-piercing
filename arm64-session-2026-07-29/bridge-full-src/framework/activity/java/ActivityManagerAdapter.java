...
public class ActivityManagerAdapter extends IActivityManager.Stub {
    ...
    // 2026-05-19: signature extended with callerOhTokenAddr.  Pass 0 if no
    // caller available (system caller path); pass the OH ability token addr
    // resolved via adapter.core.OhTokenRegistry for App-initiated calls
    // (otherwise OH AMS returns [ABMS1361]caller invalid for normal-apl
    // helloworld → StartAbility fails silently).
    ...
    // [BRIDGED] startActivity -> OH IAbilityManager.StartAbility(Want, callerToken)
    @Override
    ...
    // [BRIDGED] startActivityWithFeature -> OH IAbilityManager.StartAbility(Want, callerToken)
    @Override
    ...
    // [BRIDGED] startActivityAsUser -> OH IAbilityManager.StartAbility(Want, callerToken)
    @Override
    ...
    // [BRIDGED] startActivityAsUserWithFeature -> OH IAbilityManager.StartAbility(Want, callerToken)
    @Override
     ...
     *
     * 2026-05-19: Now resolves callerToken via adapter.core.OhTokenRegistry
     * — without it OH AMS rejects normal-apl App callers with
     * [ABMS1361]caller invalid and StartAbility silently returns rc=2097205.
     * Mirrors ArkUI UIAbility.startAbility() which passes self.token.
     *
     * @param intent   AOSP Intent describing target activity / want
     * @param resultTo Calling Activity's IBinder token (AOSP passes the
     *                 caller's mToken via this parameter even for plain
     *                 startActivity without requestCode — see
     *                 Instrumentation.execStartActivity).  Can be null
     *                 (e.g., system-initiated launches), in which case the
     *                 native path falls back to the no-caller StartAbility
     *                 overload.
     */
    private int bridgeStartAbility(Intent intent, IBinder resultTo) {
        ...
        if (resultTo != null) {
            try {
                ...
                if (ohAddr != null) {
                    callerOhTokenAddr = ohAddr.longValue();
                } else {
                    Log.w(TAG, "bridgeStartAbility: OhTokenRegistry.findOhToken returned null "
                            + "for resultTo=" + resultTo + ", falling back to no-caller path");
                }
            } catch (Throwable t) {
    ...
    public ComponentName startService(IApplicationThread caller, Intent service,
        ...
        if (service != null) {
            ...
            // startService has no Activity-token caller — pass 0 (system-caller
            // path).  Acceptable because services are not subject to the same
            // [ABMS1361] check as activity-to-activity launches.
            nativeStartAbility(want.bundleName, want.abilityName,
                    want.action, want.uri, want.extrasJson, /*callerOhTokenAddr=*/0L);
        }
    ...
    // SoundPlaybackService) are not OHOS abilities; bind them in-process via
    // InProcessServiceBinder instead of OH ConnectAbility (see that class).
    private boolean isInAppService(ComponentName comp) {
        return InProcessServiceBinder.isInApp(comp);
    }

    private int bindInProcessService(Intent service, ComponentName comp,
            IServiceConnection connection) {
        // Service lifecycle + onServiceConnected delivery must run on the main
        // (Looper) thread and asynchronously, matching the AOSP bindService
        // contract. Post and return "bind initiated".
        new android.os.Handler(android.os.Looper.getMainLooper())
                .post(new InProcessServiceBinder(service, comp, connection));
        return 1;
    }

    // [BRIDGED] bindService -> in-process (in-app service) or OH ConnectAbility
    @Override
    public int bindService(IApplicationThread caller, IBinder token, Intent service,
        ...
        if (service != null && connection != null) {
            ...
            if (isInAppService(comp)) {
                logBridged("bindService", "-> in-process in-app service " + comp);
                ServiceConnectionRegistry.getInstance().registerConnection(connection, comp);
                return bindInProcessService(service, comp, connection);
            }
    ...
    public int bindServiceInstance(IApplicationThread caller, IBinder token, Intent service,
        ...
        if (service != null && connection != null) {
            ComponentName comp = service.getComponent();
            if (isInAppService(comp)) {
                logBridged("bindServiceInstance", "-> in-process in-app service " + comp);
                ServiceConnectionRegistry.getInstance().registerConnection(connection, comp);
                return bindInProcessService(service, comp, connection);
            }
