...
public class ActivityTaskManagerAdapter extends IActivityTaskManager.Stub {
    ...
    /**
     * 2026-05-20 H-FIX-3: populated by AbilitySchedulerBridge.onScheduleAbilityTransaction
     * from OH-side LifeCycleStateInfo.missionId.  Normal-app-accessible mission-id
     * ingest point — replaces the broken nativeGetMissionIdForBundle path
     * (system_app permission required → ERR_NOT_SYSTEM_APP=2097209).  Any OH
     * launch path (launcher click / aa start / startActivity) routes through
     * ScheduleAbilityTransaction, so this single populate covers all scenarios.
     */
    public static void populateMission(String bundleName, int missionId) {
        if (bundleName == null || bundleName.isEmpty() || missionId <= 0) return;
        ActivityTaskManagerAdapter inst = getInstance();
        Integer prev = inst.mActiveMissions.put(bundleName, missionId);
        if (prev == null || prev.intValue() != missionId) {
            Log.i(TAG, "populateMission: bundle=" + bundleName + " missionId=" + missionId
                    + (prev != null ? " (was " + prev + ")" : " (new)"));
        }
    }
    ...
    public int startActivity(IApplicationThread caller, String callingPackage,
        ...
        return bridgeStartActivityWithStack(intent, flags, resultTo);
    }
     ...
     * @return result code
     */
    private int bridgeStartActivityWithStack(Intent intent, int flags, IBinder resultTo) {
        if (intent == null) {
        ...
        // 2026-05-19: resolve callerToken once — used by both nativeStartAbility
        // fallback (below) and the final new-Mission branch.  See
        // ActivityManagerAdapter.bridgeStartAbility for the OhTokenRegistry
        // lookup + null-fallback rationale.
        long callerOhTokenAddr = 0L;
        if (resultTo != null) {
            try {
                Long ohAddr = adapter.core.OhTokenRegistry.findOhToken(resultTo);
                ...
                Log.w(TAG, "bridgeStartActivityWithStack: OhTokenRegistry lookup threw: " + t, t);
            }
        }
        ...
        if (!forceNewTask && existingMissionId != null) {
            ...
            // 2026-05-20 H-FIX-3: mActiveMissions is populated by
            // AbilitySchedulerBridge.onScheduleAbilityTransaction when OH AMS
            // pushes LifeCycleStateInfo.missionId back to client (normal-app
            // accessible path).  We don't query GetMissionInfos here — that
            // API is system_app only and returns ERR_NOT_SYSTEM_APP=2097209
            // from helloworld context.
            return result;
        }
    }
