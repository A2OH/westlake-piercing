...
public class IntentWantConverter {
    ...
    public static WantParams intentToWant(Intent intent) {
        ...
        // [FIX-SHARE-CHOOSER 2026-06-06] Intent.createChooser() wraps the real
        // share intent (e.g. ACTION_SEND) as EXTRA_INTENT inside an ACTION_CHOOSER
        // intent. mapAction() has no ACTION_CHOOSER case -> bogus
        // "ohos.want.action.android.intent.action.CHOOSER" -> StartAbility fails
        // -> app crash ("Share with friends"). Unwrap to the nested intent so the
        // real ACTION_SEND maps to ohos.want.action.sendData (OHOS share). Universal
        // (every app using createChooser).
        if (intent != null && Intent.ACTION_CHOOSER.equals(intent.getAction())) {
            Intent inner = (Intent) intent.getParcelableExtra(Intent.EXTRA_INTENT);
            if (inner != null) {
                intent = inner;
            }
        }
     ...
     *
     * 2026-05-19: identity transform.  adapter's apk_installer registers
     * each AndroidManifest activity into OH BMS with its fully-qualified
     * Java class name (e.g., com.example.helloworld.SecondActivity);
     * OH AMS looks up abilities in BMS by that same name.  Earlier
     * implementations used the OH ArkUI naming convention
     * "{simpleName}Ability" which (a) drops the package prefix and (b)
     * appends an "Ability" suffix — mismatching BMS registration and
     * causing OH AMS to fall through to its StartAbilityByFreeInstall
     * path (ABMS796, error 2097152) when an App calls startActivity()
     * to a non-main activity.  Direct verification via
     *   bm dump -n com.example.helloworld | grep \"name\"
     * confirms BMS stores the full class name unchanged.
     *
     * Reverse path (mapAbilityToClassName) is also identity now — OH
     * never sends us a "{simpleName}Ability" form for Android apps, so
     * the previous "strip Ability suffix" branch was unreachable for
     * the adapter use case anyway.  Keeping both methods as a single-
     * line identity preserves call sites and makes the semantic obvious.
     */
