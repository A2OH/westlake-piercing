...
set -e
NDK=$WLROOT/ohos-sdk-6.1/linux/native; CXX=$NDK/llvm/bin/clang++
BD=$WLROOT/art-latest/build-ohos-arm64; O=$WLROOT/bridge-build-arm64/out; LIBDIR=$O/board_libs
...
$CXX --target=aarch64-linux-ohos --sysroot=$NDK/sysroot -shared -fPIC -stdlib=libc++ -fuse-ld=lld \
  -Wl,-Bsymbolic -Wl,--version-script=$WLROOT/art-latest/ziparchive_hide.map -Wl,-soname,libart.so -Wl,--allow-shlib-undefined -Wl,--allow-multiple-definition \
