...
static void nativeBindString(JNIEnv* env, jclass clazz, jlong connectionPtr,
    ...
    // WESTLAKE §455: log what actually reaches sqlite for the statement we care about. noice's
    // Presets list came back empty from
    //   SELECT * FROM ( SELECT * FROM preset WHERE name LIKE ? ORDER BY name ASC )
    // and `name LIKE ''`/NULL matches nothing while `%` matches all 4 rows, so the bound value
    // decides it. ★Filter on the SQL text — a blind call-count cap saturates on startup queries
    // long before the interesting one (exactly how libart's SCTHROW probe hid the real exception).
    {
        const char* wl_sql = sqlite3_sql(statement);
        if (wl_sql != nullptr && strstr(wl_sql, "preset") != nullptr) {
            char wl_buf[96];
            size_t wl_n = 0;
            for (jsize wl_i = 0; wl_i < valueLength && wl_n < sizeof(wl_buf) - 1; wl_i++) {
                const jchar wl_c = value[wl_i];
                wl_buf[wl_n++] = (wl_c >= 32 && wl_c < 127) ? (char) wl_c : '.';
            }
            wl_buf[wl_n] = '\0';
            fprintf(stderr, "[WESTLAKE-455] bind idx=%d len=%d value=\"%s\" sql=%.90s\n",
                    (int) index, (int) valueLength, wl_buf, wl_sql);
            fflush(stderr);
        }
    }
...
static jlong nativeExecuteForCursorWindow(JNIEnv* env, jclass clazz,
    ...
    // WESTLAKE §456: does the preset query actually deliver rows to Java? This splits a
    // SQLite/cursor problem from a UI/adapter problem — the binding is provably correct ("%%").
    {
        const char* wl_sql = sqlite3_sql(statement);
        if (wl_sql != nullptr && strstr(wl_sql, "preset") != nullptr) {
            fprintf(stderr, "[WESTLAKE-456] cursor fill: totalRows=%d addedRows=%d startPos=%d "
                    "requiredPos=%d numColumns=%d sql=%.80s\n",
                    totalRows, addedRows, startPos, requiredPos, numColumns, wl_sql);
            fflush(stderr);
        }
    }
    ...
    if (totalRows > 0 && addedRows == 0) {
        String8 msg;
        // WESTLAKE §452b: report the window geometry — "row too big" with tiny rows means the
        // window has no usable space, which is a very different bug from an actually-large row.
        fprintf(stderr, "[WESTLAKE-452] copyRow added NOTHING: window size=%zu freeSpace=%zu "
                "numColumns=%d startPos=%d requiredPos=%d totalRows=%d\n",
                window->size(), window->freeSpace(), numColumns, startPos, requiredPos, totalRows);
        fflush(stderr);
