  ...
  switch (type) {
    ...
    case kInterface:
      if ((tls_value & kInvokeInterfaceOnObjectMethodFlag) != 0) {
        ...
        // [FIX-H-RANGE] IMT corruption guard (Diag-H/I 2026-05-24): detect IMT slot
        // that holds a mirror::Object* in the dalvik-main heap range rather than an
        // ArtMethod* in LinearAlloc (LinearAlloc lives above 0xf4xxxxxx on 32-bit OHOS;
        // anything below 0x80000000 cannot be a valid ArtMethod*). Forces the existing
        // FindVirtualMethodForInterface fallback path.
        uintptr_t __fixh_imt_addr = reinterpret_cast<uintptr_t>(called_method);
        bool __fixh_in_heap = (__fixh_imt_addr >= 0x12000000UL &&
                               __fixh_imt_addr <  0x80000000UL);
        if (UNLIKELY(__fixh_in_heap)) {
          LOG(WARNING) << "[FIX-H-RANGE] IMT entry in heap range 0x" << std::hex
                       << __fixh_imt_addr << " for iface_method="
                       << interface_method->PrettyMethod()
                       << " receiver_class="
                       << (*this_object)->GetClass()->PrettyClass();
        }
        if (called_method->IsRuntimeMethod() || UNLIKELY(__fixh_in_heap)) {
