...
#include <unistd.h>   // [W15-NPE-RECOVER] write(2)
#include <cstdio>     // [W15-NPE-RECOVER] snprintf
...
bool NullPointerHandler::Action(int sig ATTRIBUTE_UNUSED, siginfo_t* info, void* context) {
  ...
  bool w15_ivm = IsValidMethod(*sp);
  {
    char w15a[160];
    int w15an = snprintf(w15a, sizeof(w15a), "[W15-ACT] enter pc=%zx fault=%zx ivm=%d\n",
        static_cast<size_t>(mc->arm_pc), static_cast<size_t>(fault_address), w15_ivm ? 1 : 0);
    for (int o = 0; o < w15an;) { ssize_t w = write(2, w15a + o, w15an - o); if (w <= 0) break; o += static_cast<int>(w); }
  }
  if (!w15_ivm) {
    // [W15-NPE-RECOVER] On our musl/OHOS substrate the frame's ArtMethod is NOT at *sp
    // for some frameless-compiled and nterp fault sites, so ART bails here and the
    // null-deref goes fatal instead of becoming the catchable NPE the app would handle.
    // Scan up-stack for the nearest valid ArtMethod frame and throw from there. This is
    // FAILURE-PATH ONLY: when *sp is valid (HelloWorld + every normal null check) nothing
    // below changes. Fully logged so the recovery can be verified/iterated.
    ArtMethod** rsp = nullptr;
    for (size_t k = 1; k <= 64; ++k) {
      if (IsValidMethod(sp[k])) {
        rsp = sp + k;
        break;
      }
    }
    {
      char w15b[200];
      int w15n = snprintf(w15b, sizeof(w15b),
          "[W15-NPE] fault@%p pc=%p sp=%p *sp-invalid; method-frame at sp+%d -> %s\n",
          reinterpret_cast<void*>(fault_address), reinterpret_cast<void*>(mc->arm_pc),
          reinterpret_cast<void*>(sp),
          (rsp != nullptr) ? static_cast<int>(rsp - sp) : -1,
          (rsp != nullptr) ? "RECOVER+throw NPE" : "no frame -> fatal");
      for (int o = 0; o < w15n;) {
        ssize_t w = write(2, w15b + o, w15n - o);
        if (w <= 0) break;
        o += static_cast<int>(w);
      }
    }
    if (rsp == nullptr) {
      return false;  // genuinely no recoverable frame -> fatal as before
    }
    sp = rsp;
    mc->arm_sp = reinterpret_cast<uintptr_t>(sp);
  }
  ...
  *reinterpret_cast<uintptr_t*>(mc->arm_sp) = return_pc;
  ...
  // [W15-REDIR-TEST REVERTED 2026-05-29] Sentinel (mc->arm_pc=0xdead0000) removed — the
  // sentinel test already PROVED OHOS sigreturn drops this edit (crash stayed at the original
  // faulting instr, never reached 0xdead0000). Real redirect restored: for the switch-interp
  // build, app-code nulls are caught explicitly before any fault, so this signal path is only
  // hit by compiled-framework nulls (still fatal until the musl sigchain fix lands).
  ...
  {
    char w15t[160];
    int w15tn = snprintf(w15t, sizeof(w15t),
        "[W15-ACT] throw-setup done, redirect pc->NPE entry, return_pc=%zx (sp=%zx)\n",
        static_cast<size_t>(return_pc), static_cast<size_t>(mc->arm_sp));
    for (int o = 0; o < w15tn;) { ssize_t w = write(2, w15t + o, w15tn - o); if (w <= 0) break; o += static_cast<int>(w); }
  }
  return true;
}
